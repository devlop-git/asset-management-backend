        ...
        if (lab === 'IGI' && cert) {
            ...
            try {
                ...
                if (report && report.length > 0) {
                    const { stonedata, media }: any = mapReportToStoneAndMedia(JSON.parse(report)[0], stock);
                    ...
                    console.log(`✅ Processed cert ${cert}`);
                } else {
                    console.warn(`⚠️ No data found for cert ${cert}`);
                }
            } catch (error) {
                console.error(`❌ Error processing cert ${cert}:`, error.message || error);
                // Optionally log or record failed certs here
            }
        }

        // Delay between processing stocks
        if (delayMs > 0) {
