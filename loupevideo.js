// // streamV360ToMp4.cjs
// // Usage:
// //   node streamV360ToMp4.cjs <CERT_ID> [out.mp4] [fps]
// // Example:
// //   node streamV360ToMp4.cjs aae74e3f-0c72-558d-b6c3-b21f00e76379 out.mp4 30
...
// const fetch = global.fetch || require("node-fetch"); // Node <18 needs: npm i node-fetch

// const CERT_ID = process.argv[2] || "aae74e3f-0c72-558d-b6c3-b21f00e76379";
...
// // Detect a frame pattern like: prefix + zero-padded index + .ext
// // Returns: { prefix, pad, ext, start, end }
// async function detectFrames(baseUrl, maxHint = 1000) {
...
//         if (isImgType(t)) {
//           // Grow 'hi' until a miss, then binary search last valid
...
// // Start ffmpeg reading frames from stdin (image2pipe) and writing MP4
...
//     "-f", "image2pipe",
//     "-i", "pipe:0",
//     // Make dimensions even for H.264 yuv420p
...
// // Stream frames (as Buffers) to ffmpeg stdin (no files saved)
// async function streamFramesToFfmpeg(pattern, ffmpegStdin) {
...
//     const ok = ffmpegStdin.write(buf, (err) => (err ? reject(err) : resolve()));
//     if (!ok) ffmpegStdin.once("drain", resolve);
//   });
...
//       if (!ct.startsWith("image/")) { console.warn(`Skip ${url}: non-image content-type: ${ct}`); continue; }

//       const buf = Buffer.from(await r.arrayBuffer());
//       if (!buf.length) { console.warn(`Skip ${url}: empty body`); continue; }
...
// (async () => {
//   try {
//     console.log("Fetching v360 info for", CERT_ID);
//     const cert = await gql(CERT_ID);
//     if (!cert?.v360?.url) throw new Error("No v360.url returned by API.");

//     console.log("Detecting frame pattern at:", cert.v360.url);
//     const maxHint = Math.max(1024, cert.v360.frame_count || 0);
//     const pattern = await detectFrames(cert.v360.url, maxHint);
//     if (!pattern) throw new Error("Could not detect frame pattern (HEAD probes failed).");
//     console.log("Detected:", pattern, "fps:", FPS);

//     // Start ffmpeg and stream frames to it
//     const ff = startFfmpeg(path.resolve(OUT), FPS);
//     const pushed = await streamFramesToFfmpeg(pattern, ff.stdin);

//     // End stdin so ffmpeg can finalize MP4
//     ff.stdin.end();
//     if (pushed === 0) {
//       ff.kill("SIGKILL");
...
//     console.error("- Node 18+ is ideal; for Node <18, `npm i node-fetch`.");
//     console.error("- Some frames may be missing; detector finds the last valid index automatically.");
//     console.error("- Try a different FPS if playback feels too fast/slow.");
//   }
// })();
...
const { spawn } = require("child_process");
const path = require("path");
const fetch = global.fetch || require("node-fetch"); // Node <18: npm i node-fetch
